...
@login_required
def start_motion_detection_view(request):
    """View para iniciar detecção de movimento"""
    if request.method == 'POST':
        try:
...
@login_required
def stop_motion_detection_view(request):
    """View para parar detecção de movimento"""
    if request.method == 'POST':
        try:
...
@login_required
def motion_detection_status_view(request):
    """View para obter status da detecção de movimento"""
    try:
